  ...
  module('projectPickerOptions', {
    setup: function setup() {
      ...
      _this.createProject = function (name, id) {
        return {
          id: id,
          avatar: "http://url/avatar/".concat(id),
          name: name,
          html: "test project <strong>".concat(name, "</strong> (<strong>").concat(name, "</strong>)")
        };
      };
  ...
  test("should format response", function () {
    ...
    testData.projects.forEach(function (testProject) {
      return ok(actualResponse[0].items().some(function (item) {
        return item.value() === testProject.id && item.icon() === testProject.avatar && item.html() === testProject.html && item.label() === "test project ".concat(testProject.name, " (").concat(testProject.name, ")");
      }), "should format response for project id:".concat(testProject.id, ", name:").concat(testProject.name));
    });
