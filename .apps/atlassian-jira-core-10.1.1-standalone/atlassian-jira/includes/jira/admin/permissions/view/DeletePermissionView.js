  ...
  return Backbone.View.extend({
    ...
    _removeGrants: function _removeGrants() {
      ...
      $.ajax({
        ...
        type: "DELETE",
        data: JSON.stringify({
          "grantsToDelete": this.grantsToRemove
        }),
    ...
    _renderDeleteGrantControl: function _renderDeleteGrantControl(grant) {
      ...
      if (grant.securityType === SecurityTypes.GROUP) {
        data = $.extend({
          emptyDisplayName: formatter.I18n.getText('admin.common.words.anyone')
        }, grant);
      } else if (grant.securityType === SecurityTypes.APPLICATION_ROLE) {
        data = $.extend({
          emptyDisplayName: formatter.I18n.getText('admin.permission.types.application.role.any')
        }, grant);
      }
