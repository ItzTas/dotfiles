AJS.test.require('jira.webresources:projectpermissions', function () {
  ...
  require(['jira/project/permissions/permissionmodel', 'jquery', 'wrm/context-path'], function (PermissionModel, $, wrmContextPath) {
    ...
    test('Should perform AJAX call to given endpoint', function () {
      var ajaxSpy = this.sandbox.spy($, 'ajax');
      this.view.ui.extToggle.click();
      sinon.assert.calledOnce(ajaxSpy);
      sinon.assert.calledWithMatch(ajaxSpy, sinon.match.has('url', wrmContextPath() + this.extPermission.endpointURI));
      sinon.assert.calledWithMatch(ajaxSpy, sinon.match.has('data', JSON.stringify(CHECKBOX_UNCHECKED)));
      sinon.assert.calledWithMatch(ajaxSpy, sinon.match.has('type', 'PUT'));
    });
    ...
    test('Should perform AJAX call to given endpoint', function () {
      ...
      sinon.assert.calledOnce(ajaxSpy);
      sinon.assert.calledWithMatch(ajaxSpy, sinon.match.has('url', wrmContextPath() + this.extPermission.endpointURI));
      sinon.assert.calledWithMatch(ajaxSpy, sinon.match.has('data', JSON.stringify(CHECKBOX_CHECKED)));
      sinon.assert.calledWithMatch(ajaxSpy, sinon.match.has('type', 'PUT'));
    });
  });
});