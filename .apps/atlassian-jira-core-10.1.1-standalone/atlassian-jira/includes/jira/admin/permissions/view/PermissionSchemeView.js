  ...
  return Backbone.View.extend({
    ...
    render: function render() {
      ...
      _.each(grouping, function (group) {
        ...
        var groupView = new PermissionsGroupView({
          model: groupModel,
          readOnly: isReadOnly
        });
    ...
    openDeletePermissionView: function openDeletePermissionView(permissionModel) {
      this.$el.append(JIRA.Templates.ProjectPermissions.deleteDialog({
        permissionName: permissionModel.get("permissionName")
      }));
      metrics.start({
    ...
    displayOperationFailureMessage: function displayOperationFailureMessage(response) {
      ...
      if (errors.length > 0) {
        ...
        var messageBody = JIRA.Templates.ProjectPermissions.renderMessageList({
          messages: errors
        });
        if (errors.length > 1) {
