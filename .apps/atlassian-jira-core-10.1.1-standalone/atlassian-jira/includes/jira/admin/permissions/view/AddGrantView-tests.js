AJS.test.require('jira.webresources:projectpermissions', function () {
  ...
  test('A user with no browse user permission shows info icon', function () {
    ...
    view.render();
    ok(view.$el.find('.grant-permission-inline-dialog').length > 0);
  });
  ...
  test('Test event was added and initial value is stored in the model', function () {
    var securityTypesThatRequiredInputValue = [SecurityTypes.USER_CF, SecurityTypes.GROUP_CF, SecurityTypes.PROJECT_ROLE, SecurityTypes.APPLICATION_ROLE, SecurityTypes.GROUP, SecurityTypes.SINGLE_USER];
