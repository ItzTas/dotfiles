 ...
 */
define('jira/shifter/shifter-dialog', ['jira/util/key-code', 'jira/jquery/deferred', 'jira/lib/class', 'jira/data/session-storage', 'jira/shifter/shifter-select', 'jira/ajs/list/group-descriptor', 'jira/ajs/list/item-descriptor-factory', 'jira/shifter/shifter-analytics', 'jquery', 'underscore'], function (keyCodes, jQueryDeferred, Class, sessionStorage, ShifterSelect, GroupDescriptor, ItemDescriptorFactory, ShifterAnalytics, jQuery, _) {
  'use strict';
  ...
  return Class.extend({
    ...
    _closeDropdownsIfNeeded: function _closeDropdownsIfNeeded() {
      jQuery('.aui-dropdown2-trigger.aui-dropdown2-active').trigger('aui-button-invoke');
    },
    ...
    _makeSuggestionsHandler: function _makeSuggestionsHandler() {
      ...
      return Class.extend({
        execute: function execute(query) {
          var suggestions = [];
          var masterDeferred = jQueryDeferred();
