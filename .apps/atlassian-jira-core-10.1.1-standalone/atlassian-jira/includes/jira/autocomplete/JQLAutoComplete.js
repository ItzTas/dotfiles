  ...
  var JQLAutoComplete = function JQLAutoComplete(options) {
    ...
    that.renderTerminologyHint = function (terminologyTerms) {
      ...
      if (terminologyTerms.length > 1) {
        var sprintNewName = StringUtil.toSentenceCase(_.findWhere(terminologyTerms, {
          originalName: 'sprint'
        }).newNamePlural);
        var epicNewName = StringUtil.toSentenceCase(_.findWhere(terminologyTerms, {
          originalName: 'epic'
        }).newNamePlural);
    ...
    that.prepareOperandSuggestions = function (canAutoComplete, fieldName, functionSuggestions, operatorSuggestions, suggestedValue) {
      if (canAutoComplete) {
        ...
        var data = {
          fieldName: Util.stripEscapeCharacters(fieldName)
        };
    ...
    that.preparePredicateSuggestions = function (predicateName, suggestedValue) {
      ...
      var data = {
        predicateName: Util.stripEscapeCharacters(predicateName)
      };
      if (result.getLastOperand() !== null || suggestedValue !== null) {
    ...
    that.saveResponse = function (val, response) {
      if (typeof val === "string" && _typeof(response) === "object") {
        if (!this.requested) {
          this.requested = {};
        }
