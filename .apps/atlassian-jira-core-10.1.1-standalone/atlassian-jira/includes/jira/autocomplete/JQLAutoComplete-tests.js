AJS.test.require(['jira.webresources:jqlautocomplete'], function () {
  ...
  var terminologyFeatureService = require('jira/terminology-feature-service');
  var $ = require('jquery');
  ...
  _testParse("\0iuyiuyiu", PARSE_FAIL);
  _testParse("aa = order by \x01", PARSE_FAIL);
  _testParse('cont\nrol = \'\'', PARSE_FAIL);
  ...
  // Represents fields data from REST API
  var JQL_FIELD_NAMES_TEST_DATA = [{
    auto: true,
    cfid: 'cf[10008]',
    displayName: 'Sprint - cf[10008]',
    ...
    auto: true,
    cfid: 'cf[10008]',
    displayName: 'Potato (Sprint) - cf[10008]',
    operators: ['=', '!=', 'in', 'not in', 'is', 'is not'],
    orderable: true,
    searchable: true,
    types: ['com.atlassian.greenhopper.service.sprint.Sprint'],
    ...
    cfid: 'cf[10006]',
    displayName: 'Epic Colour - cf[10006]',
    operators: ['=', '!=', 'in', 'not in', 'is', 'is not'],
    orderable: true,
    searchable: true,
    ...
    cfid: 'cf[10006]',
    displayName: 'Orange Colour (Epic Colour) - cf[10006]',
    operators: ['=', '!=', 'in', 'not in', 'is', 'is not'],
    orderable: true,
    searchable: true,
    types: ['java.lang.String'],
  ...
  var terminology_Entries = [{
    originalName: 'sprint',
    newName: 'Potato',
    newNamePlural: 'Buggies'
  }, {
    originalName: 'epic',
    newName: 'Orange',
    newNamePlural: 'Oranges'
  }];
  var issueTypeValuesResponse = '{"results":[{"value":"Bug","displayName":"Bug"},{"value":"Epic","displayName":"Epic"},{"value":"Epic","displayName":"Orange (Epic)"},{"value":"Story","displayName":"Story"},{"value":"Sub-task","displayName":"Sub-task"},{"value":"Task","displayName":"Task"}]}';
  var issueTypeValuesFilteredResponse = "{\"results\":[{\"value\":\"Epic\",\"displayName\":\"<b>Or</b>ange (Epic)\"}]}";
  module('Terminology Hint', {
    setup: function setup() {
      var _this = this;
      var $jqlSearchContainer = $("<textarea id=\"jql-search\"/>");
      this.sandbox = sinon.sandbox.create();
      this.clock = this.sandbox.useFakeTimers();
      this.server = this.sandbox.useFakeServer();
      this.server.autoRespond = true;
      this.server.respondWith(new RegExp('/jql/autocompletedata/suggestions\\?fieldName=issuetype&fieldValue=(.*?)&_=\\d*$'), function (xhr, value) {
        var res = value ? issueTypeValuesFilteredResponse : issueTypeValuesResponse;
        xhr.respond(200, {
          'Content-Type': 'application/json'
        }, res);
      });
      this.isTerminologyActive = true;
      this.sandbox.stub(terminologyFeatureService, 'isTerminologyActiveForUser', function () {
        return _this.isTerminologyActive;
      });
      this.sandbox.stub(terminologyFeatureService, 'getTerminologyEntries').returns(terminology_Entries);
      this.$fixture = $('#qunit-fixture');
      $jqlSearchContainer.appendTo(this.$fixture);
    },
    teardown: function teardown() {
      this.sandbox.restore();
    },
    initializeJQLAutocomplete: function initializeJQLAutocomplete(suggestionsLimit) {
      this.jqlAutoComplete = new JQLAutoComplete({
        fieldID: 'jql-search',
        // eslint-disable-next-line new-cap
        parser: new myParser(ALL_RESERVED_WORDS),
        jqlFieldNames: JQL_FIELD_NAMES_TEST_DATA,
        jqlFunctionNames: [],
  ...
  test('does not display terminology hint when terminology is not active', function () {
    this.isTerminologyActive = false;
    this.initializeJQLAutocomplete();
    this.jqlAutoComplete.dispatcher('bug');
    notOk(this.$fixture.find('.terminology-help-container').length);
  });
  test('displays terminology hint when a single term has matching suggestions', function () {
    // we don't want all the suggestions to show up, so we limit them to 2
    this.initializeJQLAutocomplete(2);
    this.jqlAutoComplete.dispatcher('bug'); // sprint is renamed to "Potato" in test data
    var $terminologyHint = this.$fixture.find('.terminology-help-container');
    var message = $terminologyHint.find('p.syntax-help span').text();
    ok($terminologyHint.length);
    equal(message, 'jira.terminology.jqlautocomplete.hint.single');
  });
  test('displays terminology hint for epic and sprints combined when there are matching suggestions', function () {
    this.initializeJQLAutocomplete();
    // since we only have 4 fields in test data and default limit is 15, all of them will show up, it doesn't matter what exactly we search for
    this.jqlAutoComplete.dispatcher('lol');
    var $terminologyHint = this.$fixture.find('.terminology-help-container');
    var message = $terminologyHint.find('p.syntax-help span').text();
    ok($terminologyHint.length);
    equal(message, 'jira.terminology.jqlautocomplete.hint.multiple');
  });
  test('displays terminology hint for issuetype value suggestions', function () {
    this.initializeJQLAutocomplete();
    this.jqlAutoComplete.field.val('issuetype = ');
    this.jqlAutoComplete.dispatcher('issuetype = ');
    this.clock.tick(50);
    var $terminologyHint = this.$fixture.find('.terminology-help-container');
    var message = $terminologyHint.find('p.syntax-help span').text();
    ok($terminologyHint.length);
  ...
  test('displays terminology hint while filtering issuetype value suggestions', function () {
    this.initializeJQLAutocomplete();
    this.jqlAutoComplete.field.val('issuetype = Or');
    this.jqlAutoComplete.dispatcher('issuetype = Or');
    this.clock.tick(50);
    var $terminologyHint = this.$fixture.find('.terminology-help-container');
    var message = $terminologyHint.find('p.syntax-help span').text();
    ok($terminologyHint.length);
