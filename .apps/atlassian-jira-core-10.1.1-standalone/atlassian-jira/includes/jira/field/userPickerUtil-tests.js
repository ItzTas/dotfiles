AJS.test.require(["jira.webresources:jira-fields", "jira.webresources:jira-global"], function () {
  ...
  test("formatResponseWithoutHTML escapes user display name and name", function () {
    ...
    var data = {
      users: [{
        name: 'Bob \x3Cscript>alert(\'Executing JS\')\x3C/script>',
        displayName: 'Nice site,  I think I\'ll take it. \x3Cscript>alert(\'Executing JS\')\x3C/script>',
        emailAddress: 'email2@qunit.com',
        avatarUrl: '/avatar2'
      }]
    };
    var query = 'site';
    ...
    equal(itemDescriptior.label(), unescapedDisplayName, "First ItemDescriptor should be \"".concat(unescapedDisplayName, "\""));
