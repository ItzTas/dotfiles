...
AJS.test.require('jira.webresources:jira-global', function () {
    ...
    setup: function setup() {
      ...
      this.testHelper = {
        ...
        respondWith: function respondWith(numberOfItems) {
          _.last(requests).respond(200, {
            "Content-Type": "application/json"
          }, this.responseBuilder(numberOfItems));
        },
        urlFromLastRequest: function urlFromLastRequest() {
          var request = _.last(requests);
          return _typeof(request) === 'object' ? request.url : '';
        },
  ...
  testCases.forEach(function (testCase) {
    test("".concat(testCase.description), function () {
      ...
      equal(this.testHelper.paramsFromLastRequest()[testCase.expectedUsernameKey], searchValue, "".concat(testCase.expectedUsernameKey, " param is set"));
