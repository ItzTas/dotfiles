  ...
  return SingleSelect.extend({
    ...
    init: function init(options) {
      ...
      function getUrl() {
        ...
        AJS.populateParameters();
        if (isIgnoreBrowseUsersPermissionFeatureFlagTurnedOn) {
          return "".concat(contextPath).concat(params.assigneeEditIssueKey ? '/rest/internal/2/users/assignee' : '/rest/internal/2/users/assignee/multiProjectSearch');
        }
        var path = params.assigneeEditIssueKey ? 'search' : 'multiProjectSearch';
        return "".concat(contextPath, "/rest/api/latest/user/assignable/").concat(path);
      }
      function formatResponse(response) {
        ...
        var groupDescriptor = new GroupDescriptor({
          weight: 1,
          // index of group in dropdown
          id: 'assignee-group-search',
          uniqueItemScope: 'container',
          replace: true,
          // Allow subsequent calls to replace model items
