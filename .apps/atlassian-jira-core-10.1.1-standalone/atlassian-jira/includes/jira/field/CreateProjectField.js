define('jira/field/create-project-field', ['jira/util/formatter', 'jira/lib/class', 'jira/ajs/dark-features', 'jira/project/project-key-generator', 'jira/project/project-sample', 'aui/inline-dialog', 'wrm/context-path', 'jquery'], function (formatter, Class, DarkFeatures, ProjectKeyGenerator, ProjectSample, InlineDialog, wrmContextPath, jQuery) {
  ...
  return Class.extend({
    ...
    init: function init(options) {
      ...
      this.keygen = new ProjectKeyGenerator({
        desiredKeyLength: 4,
      ...
      if ($keyHelpElement.length) {
        new InlineDialog($keyHelpElement, "project-key-help-popup", function (contents, trigger, show) {
    ...
    _loadExistingProjects: function _loadExistingProjects() {
      var instance = this;
    ...
    _bindNameHook: function _bindNameHook(e) {
      this._bindHook(e, this.onNameTimeout);
    },
    _bindKeyHook: function _bindKeyHook(e) {
      var el = jQuery(e.target);
      el.data("lastValue", el.val());
      this._bindHook(e, this.onKeyTimeout);
    },
    _bindHook: function _bindHook(e, func) {
      var instance = this;
      var el = jQuery(e.target);
      var _hook;
      _hook = function hook() {
        ...
        func.apply(instance);
        if (el.is(":visible")) {
          el.data("checkHook", setTimeout(_hook, instance.TIMEOUT_MS));
        }
      };
      if (!el.data("checkHook")) {
        el.data("checkHook", setTimeout(_hook, 0));
      }
    },
    _unbindHook: function _unbindHook(e) {
      var el = jQuery(e.target);
      clearTimeout(el.data("checkHook"));
      el.removeData("checkHook");
    },
    shouldUpdateKey: function shouldUpdateKey() {
      return this.$keyEditedElement.val() !== "true";
    },
    setKeyEdited: function setKeyEdited(key) {
    ...
    updateKey: function updateKey(key) {
     ...
     * @param msg
     */
    showInlineError: function showInlineError($element, initialVal, msg) {
    ...
    showInitialError: function showInitialError($element) {
      var $initialErrorElement = $element.parent().find(".initial-error");
