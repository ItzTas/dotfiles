define('jira/dashboard/dashboards-table', ['jira/util/events', 'jira/util/events/types', 'jira/util/events/reasons', 'jira/common/page', 'jira/field/favourite-picker', 'jira/autocomplete/user-autocomplete', 'jira/dialog/form-dialog2', 'jira/tabs/tab-manager', 'jira/tables/legacy-table-utils', 'wrm/context-path', 'jquery'], function (Events, Types, Reasons, Page, FavouritePicker, UserAutoComplete, FormDialog2, TabManager, TableUtils, wrmContextPath, jQuery) {
  "use strict";

  var DashboardsTable = {};
  function ajaxSearchRequest(url) {
    jQuery("#filter_search_results").empty();
    jQuery.ajax({
      method: "get",
      dataType: "html",
      ...
      success: function success(result) {
        ...
        Events.trigger(Types.NEW_CONTENT_ADDED, [$container, Reasons.dashboardsSearchRefreshed]);
        favouriteHandler();
        searchHandler();
      }
    });
  ...
  function searchHandler() {
    ...
    searchForm.submit(function () {
      ...
      ajaxSearchRequest(contextPath + "/secure/ConfigurePortalPages.jspa?" + jQuery(this).serialize());
      return false;
    });
  }
