AJS.test.require(["jira.webresources:jira-global"], function () {
  ...
  module("initDialog", {
    setup: function setup() {
      ...
      this.server.respondWith("GET", /.*WorkflowUIDispatcher.*/, [200, {
    ...
    testDialogInstancePersistence: function testDialogInstancePersistence(linkHtml, linkHtml2, actionId, firstDialogButtonId, secondDialogButtonId) {
      var dialogId = "workflow-transition-".concat(actionId, "-dialog");

      // open & close dialog 1
      $(linkHtml).appendTo($("#qunit-fixture"));
      $("#".concat(firstDialogButtonId)).click();
      this.server.respond();
      var dialog1 = Dialogs[dialogId];
      ok($("#".concat(dialogId))[0], "Dialog1 appended to DOM");
      dialog1.hide();
      ok(!$("#".concat(dialogId))[0], "Dialog1 removed from DOM");
      equal(dialog1.options.isIssueDialog, true);
      equal(dialog1.options.refreshOnSubmit, true);

      // open & close dialog 2
      $(linkHtml2).appendTo($("#qunit-fixture"));
      $("#".concat(secondDialogButtonId)).click();
      this.server.respond();
      var dialog2 = Dialogs[dialogId];
      ok($("#".concat(dialogId))[0], "Dialog2 appended to DOM");
      dialog2.hide();
      ok(!$("#".concat(dialogId))[0], "Dialog2 removed from DOM");
  ...
  var newLinkHtml = function newLinkHtml(anchorTagId, actionId, children, otherAttributes) {
    return "<a id='".concat(anchorTagId, "' class='issueaction-workflow-transition' href='/secure/WorkflowUIDispatcher.jspa?id=").concat(actionId, "&action=").concat(actionId, "' ").concat(otherAttributes, ">").concat(children, "</a>");
  };
  ...
  test("Workflow transition link handler called when clicked on a span inside transition link", function () {
    ...
    var linkHtml = newLinkHtml(anchorTagId, actionId, "<span id='".concat(childSpanId, "' class='trigger-label'>Test 2</span>"));
