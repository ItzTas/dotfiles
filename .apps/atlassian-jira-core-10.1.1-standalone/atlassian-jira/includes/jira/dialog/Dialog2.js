...
define('jira/dialog/dialog2', ['require'], function (require) {
  ...
  var dimAJS = AJS.dim;
  var undimAJS = function undimAJS() {
    AJS.undim();
  };
  ...
  var Dialog = Control.extend({
     ...
     */
    setContent: function setContent(content, decorate) {
      var node;
      if (this.resourcesReady().state() !== "resolved") {
        this._showLoadingIndicator();
        this.resourcesReady().done(this.setContent.bind(this, content, decorate));
        return;
      }
      if (!content) {
        this._contentRetrievers[this.options.type].call(this, this.setContent);
      } else if (DialogStack.current === this) {
        this.$content = content;
        this.get$popupContent().html(content);
        if (decorate !== false && this.decorateContent) {
          this.decorateContent();
        }
        if (!this.get$popup().find(".".concat(this.classNames.FOOTER_AREA)).length) {
          var $legacyFooter = this.get$popup().find('.buttons');
          this.moveFooterToDialog2($legacyFooter);
        } else if (this.get$popupContent().find(".".concat(this.classNames.FOOTER_AREA)).length) {
          // Check if the footer is inside the content - then promote it outside the main content area.
          this.get$popup().append(this.get$popupContent().find(".".concat(this.classNames.FOOTER_AREA)));
        }

        // Promote any nested HEADING_AREA in to the actual heading location.
        if ((node = this.get$popupContent().find(".".concat(this.classNames.HEADING_AREA.split(' ').join('.')))).size() > 0) {
          this.get$popupHeading().replaceWith(node);
        }

        // Remove any nested CONTENT_AREA nodes.
        if ((node = this.get$popupContent().find(".".concat(this.classNames.CONTENT_AREA.split(' ').join('.')))).size() > 0) {
        ...
        if (decorate !== false && jQuery.isFunction(this.options.onContentRefresh)) {
    ...
    moveFooterToDialog2: function moveFooterToDialog2($footerContainer) {
      ...
      this.get$popup().children(".".concat(this.classNames.FOOTER_AREA)).eq(0).remove();
      var $footer = jQuery("<div class=\"".concat(this.classNames.FOOTER_AREA, "\"></div>"));
    ...
    /**
     * Adds buttons to the footer of the dialog.
     * If the footer does not exist, it will be created.
     *
     * @param $buttons - jQuery objects representing buttons to be added to the footer
     */
    addFooterButtons: function addFooterButtons() {
      var $footer = this.get$popup().find(".".concat(this.classNames.FOOTER_AREA));
      if (!$footer.length) {
      ...
      $footer.find('.buttons').prepend($buttons);
    },
    /**
     * Adds a main action button to the footer of the dialog.
     * If the footer does not exist, it will be created.
     *
     * @param options - object containing options for the primary button
     * @param options.extraClasses - array of extra classes to be added to the primary button
     * @param options.text - text to be displayed on the primary button
     * @param options.defaultEnabled - boolean indicating whether the primary button should be enabled by default
     * @param options.id - id to be added to the primary button
     * @param callback - function to be called when the primary button is clicked
     */
    addMainActionButton: function addMainActionButton(options, callback) {
      var defaultOptions = {
        extraClasses: [],
        text: formatter.I18n.getText('common.forms.confirm'),
        defaultEnabled: true
      };
      var _options = jQuery.extend({}, defaultOptions, options);
      var $primaryButton = jQuery(aui.buttons.button({
        // eslint-disable-line no-undef
        text: _options.text,
        extraClasses: _options.extraClasses,
        id: _options.id,
        isDisabled: !_options.defaultEnabled,
        type: 'primary'
      }));
      $primaryButton.on('click', callback);
      this.addFooterButtons($primaryButton);
    },
    /**
     * Adds a cancel button to the footer of the dialog.
     * The button will close the dialog when clicked.
     *
     * @param callback - function to be called when the cancel button is clicked
     */
    addCancelButton: function addCancelButton(callback) {
      var that = this;
      var cancelFn = function cancelFn() {
        callback && callback();
        that.handleCancel();
      };
      var $cancelButton = jQuery(aui.buttons.button({
        // eslint-disable-line no-undef
        text: formatter.I18n.getText('common.words.cancel'),
        extraClasses: ['cancel'],
        type: 'link'
      }));
      $cancelButton.on('click', cancelFn.bind(this));
      this.addFooterButtons($cancelButton);
    },
    /**
     * Disables all buttons in the footer of the dialog, but the cancel button.
     */
    disableControls: function disableControls() {
      var $nonCancelButtons = this.get$popup().find(".".concat(this.classNames.FOOTER_AREA, " .buttons button")).not('.cancel');
      $nonCancelButtons.prop('disabled', true);
    },
    /**
     * Enables all buttons in the footer of the dialog.
     */
    enableControls: function enableControls() {
      var $nonCancelButtons = this.get$popup().find(".".concat(this.classNames.FOOTER_AREA, " .buttons button")).not('.cancel');
      $nonCancelButtons.prop('disabled', false);
    },
    _createFooter: function _createFooter() {
      var $footer = jQuery("<div class=\"".concat(this.classNames.FOOTER_AREA, "\"><div class=\"buttons\"></div></div>"));
      this.get$popup().append($footer);
      return $footer;
    },
    ...
    get$popupContent: function get$popupContent() {
      if (!this.$popupContent) {
        this.$popupContent = this._render("popupContent").insertAfter(this.get$popupHeading());
      }
    ...
    _onShowContent: function _onShowContent() {
      ...
      this._ellipsify();
      this._addSubmitFormAttributeWhenFormPresent();
      this.get$popup().addClass(this.classNames.CONTENT_READY);
    },
    /**
     * Adds the form attribute to the submit button if it is not already present.
     * It is done so that when DOM promotion logic moves footer outside the content area (along with moving buttons outside potential form),
     * the submit button is still associated with the form.
     * @private
     */
    _addSubmitFormAttributeWhenFormPresent: function _addSubmitFormAttributeWhenFormPresent() {
      var $popupContentForm = this.get$popupContent().find('form');
      if ($popupContentForm.length === 0) {
        return;
      }
      var $submitButton = this.get$popup().find(".".concat(this.classNames.FOOTER_AREA, " button[type=\"submit\"]"));
      var formId = $popupContentForm.attr('id');
      if (formId && !$submitButton.attr('form')) {
    ...
    hide: function hide(undim, options) {
      ...
      // In case there are multiple dialogs present, undim only when the last one is being closed.
  ...
  Dialog.getSelector = function (container) {
    ...
    return firstClass ? ".".concat(firstClass) : '';
  };
