...
define('jira/tabs/tab-manager', ['jira/util/events', 'jira/util/events/types', 'jira/util/events/reasons', 'jira/common/page', 'jira/tabs/tab-manager/util', 'jquery', 'exports'], function (Events, Types, Reasons, Page, TabManagerUtil, $, exports) {
  ...
  exports.navigationTabs = function () {
    ...
    var CONST = {
      filterTabSelector: Page.mainContentCssSelector,
      // where async response is loaded
      tabsSelector: "ul.vertical.tabs li",
      // actual tab links items
      tabsLinksSelector: "ul.vertical.tabs li a",
      // actual tab links
      requestParams: "decorator=none&contentOnly=true",
      // params to ensure response is not decorated with furniture
      stateRequestParams: "decorator=none&contentOnly=true&updateState=true",
      activeTabClass: "active",
      // class applied to selected tabs
      loadedTabClass: "loaded",
      // class applied to selected and loaded tabs
      tabParam: "filterView"
    };
    ...
    var navigateToTab = function navigateToTab(tab, historyEvent, url) {
      ...
      if (url && (activeTab.get(0) !== tab || url)) {
        ...
        xhrObject = $($.ajax({
          ...
          target: tab
        }); // lets use the throbber plugin, we will only see the throbber when the request is latent...
      }
    };
