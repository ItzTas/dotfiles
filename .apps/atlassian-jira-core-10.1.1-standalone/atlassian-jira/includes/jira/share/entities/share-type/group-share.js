define('jira/share/entities/share-type/group-share', ['jira/share/entities/display', 'jira/share/entities/share-permission', 'jira/share/i18n', 'jquery'], function (Display, SharePermission, i18n, jQuery) {
  ...
  function GroupShare(parentClass) {
    ...
    this.parentElement = document.querySelector(".".concat(parentClass));
    this.dirty = false;
  }
  GroupShare.prototype = {
    ...
    getDisplayFromUI: function getDisplayFromUI() {
      ...
      var newPermission = new SharePermission(this.type, group, null);
      return new Display(this.getDisplayString(group), this.getDescriptionString(group, true), this.singleton, newPermission);
    },
    /**
    ...
    /**
     * Returns the name of the selected group or null if no group was selected
     */
    getSelectedGroup: function getSelectedGroup() {
      if (!this.groupSelect || !this.groupSelect.options[this.groupSelect.selectedIndex]) {
        return null;
      }
      return this.groupSelect.options[this.groupSelect.selectedIndex].value;
    },
    /**
    ...
    getSubElement: function getSubElement(className) {
      return this.parentElement.querySelector(".".concat(className));
    }
  };
  return GroupShare;
});