define('jira/share/controllers/edit-shares-model', ['aui/message', 'jquery', 'jira/util/formatter'], function (AuiMessages, jQuery, formatter) {
  ...
  EditSharesModel.prototype = {
    ...
    setupDefaultShareType: function setupDefaultShareType() {
      ...
      this.getModeElementById("share_".concat(selectedType)).style.display = '';
      return selectedType;
    },
    ...
    getModeElementById: function getModeElementById(id) {
      return document.getElementById("".concat(id, "_").concat(this.mode));
    },
    removeEditorsSelect: function removeEditorsSelect(selectClass) {
      var editorsSelect = document.querySelector(".editors select.".concat(selectClass));
      if (editorsSelect) {
        editorsSelect.parentNode.removeChild(editorsSelect);
      }
    },
    animateShare: function animateShare(index) {
      var div = this.getModeElementById("share_div_".concat(index, "_inner"));
      if (div) {
    ...
    addDisplay: function addDisplay(shareDisplay, container, removeCallback) {
      ...
      container.id = "share_div_".concat(newCount, "_").concat(this.mode);
    ...
    remove: function remove(id, sharesLength) {
      var removeDiv = this.getModeElementById("share_div_".concat(id));
      if (sharesLength >= 1) {
    ...
    getSelectedType: function getSelectedType() {
      ...
      for (var i = 0; i < options.length; i++) {
        var element = this.getModeElementById("share_".concat(options[i].value));
        if (element) {
    ...
    getShareTypeSelector: function getShareTypeSelector() {
      return jQuery("#share_type_selector_".concat(this.mode));
    },
