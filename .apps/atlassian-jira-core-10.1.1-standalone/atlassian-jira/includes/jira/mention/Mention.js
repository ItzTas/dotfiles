define('jira/mention/mention', ['jira/util/key-code', 'jira/ajs/control', 'jira/dialog/dialog-stack', 'jira/mention/mention-user', 'jira/mention/mention-group', 'jira/mention/mention-matcher', 'jira/mention/scroll-pusher', 'jira/mention/contextual-mention-analytics-event', 'jira/mention/uncomplicated-inline-layer', 'jira/ajs/layer/inline-layer/standard-positioning', 'jira/ajs/layer/inline-layer/window-positioning', 'jira/ajs/dropdown/dropdown-list-item', 'jira/util/events', 'aui/progressive-data-set', 'jquery', 'underscore', 'wrm/context-path', 'jira/util/logger', 'jira/util/formatter'], function (keyCodes, Control, DialogStack, UserModel, MentionGroup, MentionMatcher, jiraScrollPusher, ContextualMentionAnalyticsEvent, UncomplicatedInlineLayer, InlineLayerStandardPositioning, InlineLayerWindowPositioning, DropdownListItem, Events, ProgressiveDataSet, jQuery, _, contextPath, logger, formatter) {
  ...
  function getUrl(issueKey, useNewEndpoint) {
    if (useNewEndpoint) {
      return "".concat(contextPath(), "/rest/internal/2/users/mention");
    }
    if (isRolesEnabled(issueKey)) {
      return "".concat(contextPath(), "/rest/internal/2/user/mention/search");
    } else {
      return "".concat(contextPath(), "/rest/api/2/user/viewissue/search");
    }
  }
  ...
  return Control.extend({
    ...
    /**
     * Returns target at which mentions should be rendered. Best case scenario is to render mentions at the caret position.
     * If for some reason the caret position is not available, fallback to rendering mentions under textarea.
     *
     * @returns {jQuery}
     * @private
     */
    _getMentionsOffsetTarget: function _getMentionsOffsetTarget() {
      var _this3 = this;
      var getPhantomCaret = function getPhantomCaret() {
        try {
          var editorAPI = _this3._getEditorAPI();
          return editorAPI && editorAPI.phantomCaret.queryPhantomCaretInDOM();
        } catch (_) {
          return undefined;
        }
      };
      try {
        var phantomCaret = getPhantomCaret();
    ...
    textarea: function textarea(_textarea) {
      ...
      this.layerController = new UncomplicatedInlineLayer({
        ...
        allowDownsize: true,
        positioningController: positioningController || windowPosController,
        // Will be undefined if no push-scroll, that is ok
    ...
    updateMentionsStatus: function updateMentionsStatus(areResultsUpdated) {
      ...
      mentionsStatusTimeoutId = setTimeout(function () {
        ...
        if (instance.layerController.isVisible()) {
          ...
          assistiveHelper.html("".concat(dropdownExpandedInformation, " ").concat(results, " ").concat(selectedUserName).trim());
      ...
      if (issueKey) {
        if (!seedData[issueKey]) {
          seedData[issueKey] = jQuery.ajax({
            method: 'GET',
            url: "".concat(contextPath()).concat(this.useNewEndpoint ? '/rest/internal/2/users/mention' : '/rest/internal/2/user/mention/search'),
...
define('jira/mention/contextual-mention-analytics-event', ['jira/util/key-code', 'jira/analytics', 'underscore'], function (keyCodes, analytics, _) {
  ...
  var contextualMentionIsBeingLookedUpEvent = _.debounce(function (username, keyCode, ctrlKey, metaKey) {
    ...
    if (isBackSpaceOrSelectAllKey === false && isUserLookingForContextualMention) {
      analytics.send({
        name: 'issue.comment.contextualMention.lookup'
      });
    }
  }, 500);
  return {
    fireAcceptedContextualMentionAnalyticsEvent: function fireAcceptedContextualMentionAnalyticsEvent() {
      analytics.send({
        name: 'issue.comment.contextualMention.accepted'
      });
    },
    ...
    fireUserMayAcceptSuggestionByUsingContextualMentionEvent: function fireUserMayAcceptSuggestionByUsingContextualMentionEvent(username) {
      _.any(["assignee", "reporter"], function (contextualMention) {
        if (contextualMention.indexOf(username) === 0 && contextualMention !== username) {
          analytics.send({
            name: 'issue.comment.contextualMention.mayAccepted'
          });
          return true;
        }
      });
