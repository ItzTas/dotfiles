...
define("jira/viewissue/tabs", ["wrm/require", 'jira/util/logger', "jquery", "jira/jquery/deferred", "jira/util/data/meta", "jira/dialog/form-dialog2", "jira/ajs/ajax/smart-ajax", "jira/userhover/userhover", "jira/viewissue/tabs/analytics"], function (WRMRequire, logger, jQuery, jQueryDeferred, Meta, FormDialog2, SmartAjax, Userhover, Analytics) {
  ...
  function dispatchIssueTabErrorEvent(smartAjaxResult, activeTabKey) {
    var errorPopup = new FormDialog2({
  ...
  function enableAjaxOnLinks(context) {
    ...
    jQuery(context).find(AJAX_LINK_SELECTOR).click(function (event) {
      ...
      var clickedTabID = $trigger.attr('id');
      var clickedTabAreiaControls = $trigger.attr('aria-controls');
      if (isSortingButton) {
        ...
        handleLoading($trigger, activeTabKey, event);
        var tabPanelWrapper = jQuery('.issuePanelWrapper');
        tabPanelWrapper.attr("aria-labelledby", clickedTabID);
        tabPanelWrapper.attr("id", clickedTabAreiaControls);
      }
    });
  }
  function handleLoading($trigger, activeTabKey, event) {
    ...
    handleAjaxContentsLoading(activeTabKey, $trigger.data('ajax') || $trigger.attr('href')).done(function ($container) {
      ...
      //if user triggered action by mouse, we don't get focus
      ...
      if (loadingTabKey) {
        ...
        var $activeTab = $container.find("#" + $trigger.attr("id"));
        if ($activeTab.length) {
          $activeTab.focus();
        }
      } else {
        //focus into sort action
        var $sortAction = $container.find(".sortwrap > :first-child");
        if ($sortAction.length) {
          $sortAction.focus();
        }
      }
  ...
  function handleAjaxContentsLoading(activeTabKey, loadingUrl) {
    ...
    var xhr = SmartAjax.makeRequest({
      ...
      // needed for the ViewIssue action to return only the activity panel
  ...
  function enableArrowNavigation() {
    var $menuItems = jQuery('.menu-item a');
    jQuery(document).on('keydown', function (e) {
      var $focusedElement = jQuery(document.activeElement);
      if ($focusedElement.is('.menu-item a')) {
        var currentIndex = $menuItems.index($focusedElement);
        var key = e.key;
        if (key === "ArrowRight" || key === "ArrowLeft") {
          ...
          e.preventDefault();
          // Optionally update ARIA attributes for better accessibility
          $menuItems.attr('tabindex', '-1');
          $menuItems.eq(newIndex).attr('tabindex', '0');
        }
      }
    });
  }
  ...
  onTabReady(enableArrowNavigation);
