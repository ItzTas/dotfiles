define('jira/project/browse/router', ['underscore', 'jira/backbone-1.3.3', 'jira/util/navigation'], function (_, Backbone, Navigation) {
  ...
  return Backbone.Router.extend({
    ...
    handleRouteChange: function handleRouteChange() {
      ...
      if (params.contains) {
        this.application.filter.set('contains', params.contains, {
          silent: true
        });
      }
      if (params.category !== false) {
        this.application.filter.set('category', params.category.toJSON(), {
          silent: true
        });
      }
      if (params.projectType !== false) {
        this.application.filter.set('projectType', params.projectType.toJSON(), {
          silent: true
        });
      }
      if (params.sortColumn) {
        this.application.filter.set('sortColumn', params.sortColumn, {
          silent: true
        });
      }
      if (params.sortOrder) {
        this.application.filter.set('sortOrder', params.sortOrder, {
          silent: true
        });
      }
