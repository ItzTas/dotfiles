define('jira/project/browse/projectview', ['jquery', 'jira/marionette-4.1', 'jira/util/data/meta', 'jira/moment', 'wrm/context-path', 'aui/flag', 'jira/ajs/ajax/ajax-util', 'jira/dialog/form-dialog2'], function ($, Marionette, meta, moment, contextPath, flag, ajaxUtil, FormDialog2) {
  'use strict';
  ...
  return Marionette.View.extend({
    ...
    archiveProject: function archiveProject(event) {
      ...
      var dialog = new FormDialog2({
        id: "".concat(event.target.id, "-dialog"),
    ...
    restoreProject: function restoreProject(event) {
      ...
      $.ajax({
        url: contextPath() + "/rest/api/2/project/".concat(projectKey, "/restore"),
        type: 'PUT',
        dataType: 'text'
      }).success(function () {
        // little work around to remove model from collection without sending a request/accessing collection from model
