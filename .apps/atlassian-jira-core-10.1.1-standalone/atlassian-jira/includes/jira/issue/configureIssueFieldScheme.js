...
define('jira/issue/configureissuefieldscheme', ['underscore', 'jquery', 'marionette', 'jira/analytics'], function (_, $, Marionette, analytics) {
  ...
  return Marionette.View.extend({
    ...
    initialize: function initialize(options) {
       ...
       */
      this.bindUIElements();
      this.optionsKey = options.optionsKey || 'selectedOptions';
    ...
    getSelectedOptions: function getSelectedOptions() {
      return this.ui.selectedOptions.sortable("serialize", {
        key: this.optionsKey
      }) || "".concat(this.optionsKey, "=");
    },
    /**
    ...
    restrictOptions: function restrictOptions() {
      var queryString = this.getSelectedOptions().replace(new RegExp("".concat(this.optionsKey, "="), 'g'), '');
      ...
      var customOptions = Array.prototype.slice.call(selectElement.options, 1);
      customOptions.forEach(function (option) {
        var $option = $(option);
        if (_.contains(selectedOptions, option.value)) {
          $option.prop('disabled', false).removeClass('hidden');
        } else {
          if (option.selected) {
            option.selected = false;
            defaultOption.selected = true;
          }
          $option.prop('disabled', true).addClass('hidden');
        }
      });
    },
    /**
