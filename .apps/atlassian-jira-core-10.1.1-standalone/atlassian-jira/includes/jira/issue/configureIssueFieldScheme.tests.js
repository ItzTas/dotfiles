AJS.test.require(['jira.webresources:configure-issue-field-scheme'], function () {
  ...
  test('Form data is updated correctly before submission', function () {
    ...
    ok(this.issueFieldView.$el.attr('action').endsWith('?selectedOptions=1&selectedOptions=2'));
  });
  test('Check whether hidden option are also disabled (needed for IE11)', function () {
    this.restrictOptionsSpy.reset(); // view init called this method

    this.issueFieldView.ui.addAllButton.click();
    ok(this.restrictOptionsSpy.calledOnce);
    var disabledDefaultOptions = this.issueFieldView.ui.defaultOptionSelect.find('option:disabled').map(function (i, el) {
      return el.id;
    }).get();
    deepEqual(disabledDefaultOptions, []);
    this.issueFieldView.ui.removeAllButton.click();
    ok(this.restrictOptionsSpy.calledTwice);
    disabledDefaultOptions = this.issueFieldView.ui.defaultOptionSelect.find('option:disabled').map(function (i, el) {
      return el.id;
    }).get();
    deepEqual(disabledDefaultOptions, ['1', '2']);
  });
});