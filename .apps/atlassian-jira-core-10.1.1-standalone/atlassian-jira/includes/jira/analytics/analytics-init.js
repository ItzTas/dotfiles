require(['jquery', 'jira/analytics', 'jira/util/data/meta'], function ($, analytics, meta) {
  ...
  function bindEvents() {
    ...
    $("[data-track-pageview]").each(function (idx, element) {
      analytics.send({
        name: addPrefixToAnalytics($(element).data("track-pageview"))
      });
    });
  }
  ...
  $(document).on("click", "[data-track-click]", function () {
    analytics.send({
      name: addPrefixToAnalytics($(this).data("track-click"))
    });
  });
  $(document).on("auxclick", "[data-track-auxclick]", function () {
    analytics.send({
      name: addPrefixToAnalytics($(this).data("track-auxclick"))
    });
  });
