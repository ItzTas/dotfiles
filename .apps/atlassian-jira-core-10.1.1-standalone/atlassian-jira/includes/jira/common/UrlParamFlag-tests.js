AJS.test.require(['jira.webresources:urlParamFlag'], function () {
  ...
  [{
    ...
    test("should handle url ".concat(data.url, " with parameter name ").concat(data.parameterName, " so that expected parameter value/flag is: ").concat(data.expectedParameterValue ? data.expectedParameterValue : 'not handled'), function () {
      ...
      if (data.expectedParameterValue) {
        sinon.assert.calledWith(this.showMsg, null, "flagContent for: ".concat(data.expectedParameterValue), {
          closeable: true,
          close: 'auto',
          type: 'info'
        });
