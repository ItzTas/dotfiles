define('jira/flag', ['jira/util/logger', 'aui/flag', 'jira/data/session-storage', 'jira/ajs/ajax/smart-ajax', 'underscore', 'jquery', 'wrm/data', 'wrm/context-path'], function (logger, flag, sessionStorage, smartAjax, _, $, wrmData, contextPath) {
  'use strict';
  ...
  function showMsg(title, body, options) {
    options = options || {};
    var settings = _.extend({}, defaults, typeDefaults[options.type], options, {
      title: title ? title : '',
      body: body ? body : ''
    });
  ...
  function showErrorMsg(title, body, options) {
    return showMsg(title, body, _.extend({}, options, {
      type: 'error'
    }));
  }
