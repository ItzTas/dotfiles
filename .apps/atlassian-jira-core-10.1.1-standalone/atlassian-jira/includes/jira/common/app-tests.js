    ...
    setup: function setup() {
      ...
      this.assertElementHasTooltip = function (name, $el, hasTooltip, assert) {
        ...
        setTimeout(function tooltipAttached() {
          if (hasTooltip) {
            sinon.assert.calledOnce(this.tooltipStub, "".concat(name, " should have tooltip initialized"));
            ...
            sinon.assert.notCalled(this.tooltipStub, "".concat(name, " should not have tooltip initialized"));
            done();
          }
        }.bind(_this), 10);
      };
