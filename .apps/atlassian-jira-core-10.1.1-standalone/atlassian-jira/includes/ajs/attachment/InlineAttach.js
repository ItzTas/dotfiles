define('jira/attachment/inline-attach', ['jira/util/urls', 'jira/util/formatter', 'jira/lib/class', 'jira/xsrf', 'jira/util/navigator', 'jquery', 'wrm/context-path'], function (urls, formatter, Class, XSRF, Navigator, jQuery, wrmContextPath) {
  'use strict';
  ...
  InlineAttach.FormPresenter = InlineAttach.Presenter.extend({
    ...
    _createSubmitData: function _createSubmitData() {
      var data = {
        ...
        formToken: this.form.getFormToken()
      };
  ...
  InlineAttach.UploadProgress = Class.extend({
    ...
    _content: function _content(rem) {
      this.$content.text(rem);
      return this;
    },
    _addRate: function _addRate(rate) {
  ...
  InlineAttach.UnknownProgress = Class.extend({
    ...
    _content: function _content(text) {
  ...
  InlineAttach.Form = Class.extend({
    ...
    getAtlToken: function getAtlToken() {
      var $atlToken = this.$form.find("input[name='atl_token']");
      if ($atlToken.length > 0) {
        return $atlToken.val();
      } else {
        return urls.atl_token();
      }
    },
    setAtlToken: function setAtlToken(token) {
    ...
    getFormToken: function getFormToken() {
     ...
     */
    addProgress: function addProgress(file) {
     ...
     */
    addStaticProgress: function addStaticProgress(fileName) {
     ...
     */
    addTemporaryFileCheckbox: function addTemporaryFileCheckbox(value, name, replaceObj, file) {
     ...
     */
    addLocalThumbnailImage: function addLocalThumbnailImage(name, file) {
      var $thumbNail = null;
    ...
    getAspectRatio: function getAspectRatio(maxWidth, maxHeight, origWidth, origHeight) {
     ...
     */
    addErrorWithFileName: function addErrorWithFileName(error, fileName, replaceObj) {
     ...
     * @param replaceObj if non falsy, the error will replace the passed object
     */
    addError: function addError(error, replaceObj) {
      var $element = InlineAttach.Renderers.container();
     ...
     * @return the old file input.
     */
    cloneFileInput: function cloneFileInput() {
  ...
  InlineAttach.FormUpload = Class.extend({
    ...
    _addToBody: function _addToBody($form) {
  ...
  InlineAttach.AjaxUpload = Class.extend({
    ...
    _clienterror: function _clienterror(e, file) {
     ...
     */
    _upload: function _upload(event) {
  ...
  jQuery.extend(InlineAttach.AjaxUpload, /** @lends InlineAttach.AjaxUpload */{
    ...
    isSupported: function isSupported() {
      return InlineAttach.AjaxUpload._fileApiSupport(window) && InlineAttach.AjaxUpload._xhrSupport();
    },
    /**
    ...
    getClientErrorMessage: function getClientErrorMessage(e, file) {
      //JRADEV-5679:
    ...
    _xhrSupport: function _xhrSupport() {
      try {
    ...
    _fileApiSupport: function _fileApiSupport(window) {
  ...
  InlineAttach.Timer = Class.extend({
    init: function init(callback, scope) {
     ...
     */
    schedule: function schedule(timeout) {
      this.cancel();
    ...
    _startTimeout: function _startTimeout(fn, timeout) {
      return window.setTimeout(fn, timeout);
    },
    _endTimeout: function _endTimeout(id) {
  ...
  InlineAttach.Text = function () {
    ...
    return /** @lends InlineAttach.Text */{
      ...
      _classifySize: function _classifySize(size) {
        var base;
        if (size <= kBMax) {
          base = kB;
        } else if (size <= MBMax) {
       ...
       */
      rate: function rate(bytesPerSecond) {
      ...
      _toDisplay: function _toDisplay(number) {
