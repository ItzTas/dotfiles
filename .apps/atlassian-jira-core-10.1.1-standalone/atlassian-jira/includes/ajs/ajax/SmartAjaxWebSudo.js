 ...
 */
define('jira/ajs/ajax/smart-ajax/web-sudo', ['jira/jquery/deferred', 'jira/ajs/ajax/smart-ajax', 'jira/ajs/ajax/ajax-util', 'jira/dialog/dialog', 'jira/dialog/form-dialog2', 'jira/util/browser', 'wrm/context-path', 'jquery'], function (jQueryDeferred, SmartAjax, AjaxUtil, Dialog, FormDialog2, Browser, wrmContextPath, $) {
  'use strict';
  ...
  return /** @alias module:jira/ajs/ajax/smart-ajax/web-sudo */{
    ...
    showWebSudoDialog: function showWebSudoDialog(options) {
      ...
      var dialog = new FormDialog2({
        ...
        submitHandler: function submitHandler(e, closeCallback) {
          e.preventDefault();
          var form = $(e.target);
          SmartAjax.makeRequest({
            ...
            complete: function complete(xhr) {
              if (xhr.getResponseHeader("X-Atlassian-WebSudo") === "Has-Authentication") {
                ...
                dialog.setContent(xhr.responseText, true);
              }
            }
          });
        }
    ...
    handleWebSudoError: function handleWebSudoError(ajaxOptions, dialogOptions, xhr, statusText, smartAjaxResult) {
      var newOptions = {
        success: function success(closeFunction) {
