AJS.test.require(["jira.webresources:util"], function () {
  ...
  module("AjaxUtil");
  var createXHR = function createXHR(status, response) {
    return {
      status: status,
  ...
  test("getErrorMessageFromXHR uses ErrorCollection in body to generate message", function () {
    equal(AjaxUtil.getErrorMessageFromXHR(createXHR(401, {
      errorMessages: ["Error"]
    })), "Error", "Single errorMessage used.");
    equal(AjaxUtil.getErrorMessageFromXHR(createXHR(401, {
      errorMessages: ["Error", "Error 2"]
    })), "Error Error 2", "Multiple errorMessages used.");
    equal(AjaxUtil.getErrorMessageFromXHR(createXHR(401, {
      errors: {
        error: "Error"
      }
    })), "Error", "Single error used.");
    equal(AjaxUtil.getErrorMessageFromXHR(createXHR(401, {
      errors: {
        error: "Error",
        error2: "Error 2"
      }
    })), "Error Error 2", "Multiple errors used.");
    ...
    })), "Error Error 2", "Multiple errors used when errorMessages empty");

    //Ignore error messages.
    equal(AjaxUtil.getErrorMessageFromXHR(createXHR(401, {
      errorMessages: ["NonEmpty"],
      errors: {
        error: "Error",
        error2: "Error 2"
      }
    })), "NonEmpty", "Ignore error messages when errorMessage available.");
  });
  test("getErrorMessageFromXHR uses XHR to generate message when ErrorCollection not available.", function () {
    equal(AjaxUtil.getErrorMessageFromXHR(createXHR(401, undefined)), "common.ajax.unauthorised.alert", "Error message returned when no error collection.");
    equal(AjaxUtil.getErrorMessageFromXHR(createXHR(204, "<xml>badErrorCollection<a></a></xml>")), "common.ajax.servererror", "Error message returned with bad error collection.");
  });
});