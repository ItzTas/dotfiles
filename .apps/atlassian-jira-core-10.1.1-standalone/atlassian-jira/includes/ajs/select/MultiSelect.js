  ...
  return QueryableDropdownSelect.extend({
    ...
    _createFurniture: function _createFurniture(disabled) {
      ...
      if (disabled) {
        ...
        this.$label = jQuery("label[for=\"".concat(jQuery.escapeSelector(id), "\"]"));
        if (this.$label.length) {
          this.$label.attr('for', this.$field.attr('id'));
        } else {
          this.$label = jQuery("label[for=\"".concat(jQuery.escapeSelector(this.$field.attr('id')), "\"]"));
        }
        if (this.$field.attr('role') === 'combobox') {
          if (ariaDescribedBy) {
            this.ariaDescriptionForCombobox = jQuery("#".concat(jQuery.escapeSelector(ariaDescribedBy))).text();
          }
    ...
    updateAriaLabel: function updateAriaLabel() {
      // There should be no such custom aria-labels but we can't just remove it as this component is used all over the project
      // it should be removed over time
      if (!this.model.$element.hasClass("no-custom-aria-label")) {
        ...
        var updateLabel = function () {
          ...
          this.$field.attr("aria-label", shouldPlaceAriaLabel ? "".concat(labelText, "- ").concat(selectedValuesText, ". ").concat(descriptionText) : null);
        }.bind(this);
