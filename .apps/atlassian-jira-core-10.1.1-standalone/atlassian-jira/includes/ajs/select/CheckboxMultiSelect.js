  ...
  return QueryableDropdownSelect.extend({
    ...
    _getCustomRenders: function _getCustomRenders() {
      return {
        ...
        // override default suggestion renderer
    ...
    _renders: {
      ...
      suggestionItemElement: function suggestionItemElement(descriptor, replacementText) {
        ...
        var onIconErrorEvent = descriptorFallbackIcon && descriptorFallbackIcon !== "none" ? " onerror=\"this.onerror=null;this.src='".concat(descriptorFallbackIcon, "';\"") : '';
        var image = '';
        if (imageShouldBeRendered) {
          image = "<img src=".concat(descriptorIcon, " \n                    ").concat(onIconErrorEvent, " \n                    loading='lazy' \n                    alt='' \n                    height='16' width='16' \n                    class='icon").concat(descriptor.iconType() === 'rounded' ? ' rounded' : '', "' \n                    align='absmiddle' />");
        }
        return jQuery("\n                    <li class='check-list-item".concat(descriptorDisabled ? ' disabled' : '', " ").concat(descriptorStyleClass || '', "' id='").concat(descriptorValue + '-' + this.options.id, "'>\n                    <label class='item-label checkbox' original-title='").concat(descriptorTitle, "' data-descriptor-title='").concat(descriptorTitle, "'>\n                        <input type='checkbox'").concat(descriptorDisabled ? ' disabled="disabled"' : '', " value='").concat(descriptorValue, "'").concat(descriptorSelected ? " checked='checked'" : '', " />\n                        ").concat(image, "\n                        ").concat(descriptorCopy, "\n                    </label>\n                    </li>"));
      },
