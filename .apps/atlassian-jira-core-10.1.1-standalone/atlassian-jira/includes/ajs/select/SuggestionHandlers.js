  ...
  return SelectSuggestHandler.extend({
    ...
    formatSuggestions: function formatSuggestions(descriptors, query) {
      ...
      if (query.length > 0) {
        ...
        descriptors = [new GroupDescriptor({
          items: items,
  ...
  return Class.extend({
    ...
    init: function init(options, model, containsStaticSuggestions) {
      this.options = _.extend({
        keyInputPeriod: 150,
        // Wait this long between key strokes before going to server
        minQueryLength: 1,
        // Need these many characters before we go to server
        data: {},
        dataType: "json"
      }, options);
      if (model) {
      ...
      ajaxOptions.error = function (xhr, textStatus, msg, smartAjaxResult) {
        if (!smartAjaxResult.aborted) {
          if (originalError) {
            ...
            alert(SmartAjax.buildSimpleErrorContent(smartAjaxResult, {
              alert: true
            }));
          }
          deferred.resolve();
