define('jira/ajs/select/security/default-comment-security-level-control', ['jira/util/formatter', 'jira/ajs/select/security/default-comment-security-level-model', 'jira/ajs/ajax/smart-ajax', 'jira/dialog/form-dialog2', 'jira/lib/class', 'jquery', "underscore", "wrm/data"], function (formatter, DefaultCommentSecurityLevelModel, SmartAjax, FormDialog2, Class, jQuery, _, wrmData) {
  'use strict';

  var helpLink = wrmData.claim('jira.core:default-comment-security-level-data.DefaultCommentSecurityLevelHelpLink');
  ...
  return Class.extend({
    ...
    _applyDefaultToSelection: function _applyDefaultToSelection(lvlObj) {
      if (!this.selectionSpi.hasSecurityLevel(lvlObj.level)) {
        ...
        this.$errorSpan.html(JIRA.Templates.CommentSecurityLevel.unavailable({
          'name': lvlObj.levelName
        }));
      } else {
