define("jira/setup/setup-tracker", ['jira/util/data/meta', "jira/jquery/deferred", "jquery", "underscore"], function (Meta, Deferred, $, _) {
  ...
  function sendUserClickedOnMigrationsLink() {
    var extParams = $.extend(getDefaultParams(), {
      "migrationsLinkClicked": "true"
    });
    return insertIframe(dataToUrl(extParams));
  }
  function sendUserClickedOnConnectingJiraToDatabaseLink() {
    var extParams = $.extend(getDefaultParams(), {
      "connectingJiraToDatabaseLinkClicked": "true"
    });
    return insertIframe(dataToUrl(extParams));
  }
  function sendUserPickedDatabase(dbType) {
    var dbType = dbType || "";
    var extParams = $.extend(getDefaultParams(), {
      "databaseOptionPicked": dbType
    });
    return insertIframe(dataToUrl(extParams));
  }
  return {
    ...
    sendUserClickedOnMigrationsLink: sendUserClickedOnMigrationsLink,
    sendUserClickedOnConnectingJiraToDatabaseLink: sendUserClickedOnConnectingJiraToDatabaseLink,
    sendUserPickedDatabase: sendUserPickedDatabase
  };
});