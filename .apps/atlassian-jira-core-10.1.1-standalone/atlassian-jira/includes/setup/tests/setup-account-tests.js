AJS.test.require("jira.webresources:jira-setup", function () {
  ...
  module("JIRA Instant Setup Admin Account", {
    ...
    initializeView: function initializeView() {
  ...
  test("error flag should be displayed when no license is present", function () {
    ...
    assert.calledWith(Flag.showErrorMsg, flagTexts.errorTitle, flagTexts.errorContent, match({
      close: "never"
    }));
  });
  ...
  test("success flag should be displayed when license is provided", function () {
    ...
    assert.calledWith(Flag.showSuccessMsg, flagTexts.successTitle, flagTexts.successContent, match({
      close: "auto"
    }));
  });
  ...
  test("email is invalid", function () {
    ...
    _.each([
    // invalid emails
    "aString", "aStringFinishingWith@", "aStringWith@butEndsIn.", "a string containing @ but also space"], _.bind(function (data) {
  ...
  test("username is too long", function () {
    // when
    this.initializeView();
    setFieldValue(this.testObj.ui.username, _.range(View.MAX_USERNAME_LEN + 1).map(function () {
      return "x";
    }).join(""));
    ...
    // but when
    setFieldValue(this.testObj.ui.username, _.range(View.MAX_USERNAME_LEN).map(function () {
      return "x";
    }).join(""));
  ...
  test("username is invalid", function () {
    ...
    _.each([
    // invalid usernames
    "aStringContaining&", "aStringContaining<", "aStringContaining>", "a string containing space"], _.bind(function (data) {
