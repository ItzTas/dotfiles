AJS.test.require("jira.webresources:jira-setup", function () {
  ...
  module("JIRA setup finishing notifications", {
    ...
    setup: function setup() {
      ...
      $("#qunit-fixture").append($("<div></div>").addClass(this.viewClassname).html(markup));
    },
    initializeView: function initializeView() {
      this.view = new View({
        el: "." + this.viewClassname
      });
      this.sandbox.stub(this.view, "_navigateToErrorPage");
    },
    ...
    respondWithNotFoundError: function respondWithNotFoundError() {
      this.sandbox.server.requests[0].respond(404, {
        "Content-Type": "application/json"
      }, JSON.stringify({}));
    },
    respondWithServerError: function respondWithServerError() {
      this.sandbox.server.requests[0].respond(503, {
        "Content-Type": "application/json"
      }, JSON.stringify({}));
    },
    respondWithSuccess: function respondWithSuccess() {
      this.sandbox.server.requests[0].respond(200, {
        "Content-Type": "application/json"
      }, JSON.stringify({
        steps: this.steps.finishingPending
      }));
    },
    assertSetupLevel: function assertSetupLevel(level) {
  ...
  test("triggerSetup redirects to an error page if server responds with 5xx", function () {
    this.initializeView();
    this.view.triggerSetup();
    this.respondWithServerError();
    sinon.assert.calledOnce(this.view._navigateToErrorPage);
  });
  test("_pullState redirects to an error page if server responds with 5xx", function () {
    this.initializeView();
    this.view._pullState();
    this.respondWithServerError();
    sinon.assert.calledOnce(this.view._navigateToErrorPage);
  });
  test("_makeRequestFinishingSetup redirects to an error page if server responds with 5xx", function () {
    this.initializeView();
