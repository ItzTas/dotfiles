    ...
    that.renderSuggestions = function (response) {
      ...
      if (results && results.length > 0) {
        ...
        jQuery(results).each(function (index, snapshot) {
          ...
          suggestionNodes.push([jQuery("<li title=\"".concat(snapshot.absolutePath, "\"/>")).html("<span class=\"aui-lozenge aui-lozenge-subtle\">".concat(getSnapshotDate(snapshot.timestamp), "</span> ").concat(getSnapshotName(snapshot.absolutePath))).appendTo(resultsContainer), snapshot.absolutePath]);
        });
      }
