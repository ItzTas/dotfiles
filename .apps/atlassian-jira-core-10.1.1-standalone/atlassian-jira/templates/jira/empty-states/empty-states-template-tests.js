AJS.test.require(['jira.webresources:empty-states', 'jira.webresources:wrm-context-path'], function () {
  ...
  require(['jira/empty-states/template', 'wrm/context-path'], function (template, contextPath) {
    ...
    var IMAGE_SELECTOR = "".concat(CONTAINER_SELECTOR, " .empty-state__spot");
    var MESSAGE_SELECTOR = "".concat(CONTAINER_SELECTOR, " .empty-state__message");
    var HINT_SELECTOR = "".concat(CONTAINER_SELECTOR, " .empty-state__hint");
    ...
    test('Should use a default image when $imageURL not provided', function () {
      this.fixture.innerHTML = template.render({
        message: ''
      });
      assertImage(this.fixture, "".concat(contextPath(), "/images/empty-states/no-results.svg"));
    });
    ...
    ['some-css-class', 'class1 class2', 'class3 class2 class1', undefined, ''].forEach(function (extraClasses) {
      test("Should render the empty state container with correct css classes for $extraClasses=".concat(extraClasses), function () {
        this.fixture.innerHTML = template.render({
          message: '',
    ...
    ['No issues were found to match your search', '<strong>markup should be escaped here</strong>', ''].forEach(function (message) {
      test("Should render the empty state message for $message=".concat(message), function () {
        this.fixture.innerHTML = template.render({
          message: message
        });
    ...
    hints.forEach(function (hint) {
      test("Should render the empty state hint for $hint=".concat(hint, " with not provided 'noAutoescapeHint'"), function () {
        this.fixture.innerHTML = template.render({
          message: '',
          hint: hint
        });
    ...
    hints.forEach(function (hint) {
      test("Should render the empty state hint for $hint=".concat(hint, " with $noAutoescapeHint=true"), function () {
        this.fixture.innerHTML = template.render({
          message: '',
          hint: hint,
          noAutoescapeHint: true
        });
        var hintEl = this.fixture.querySelector(HINT_SELECTOR);
        ok(hintEl, 'Could not find `.empty-state__hint` element');
