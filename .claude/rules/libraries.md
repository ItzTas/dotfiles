...
project or recommending one to me** — in any language, any package manager. The point is simple:
...
4. **Successor.** If it's deprecated, find what replaced it and check *that* one the same way —
   deprecation chains are common (a successor can itself be abandoned).
...
current facts, actually go get them — registry CLI, `WebFetch`/`WebSearch` on the repo and registry
page, or the MCP tools available in the session. Don't answer from memory alone.
...
- Exception: if I'm already editing that dependency's usage or bumping it, raise it there and then
  — that's the cheap moment to migrate.
