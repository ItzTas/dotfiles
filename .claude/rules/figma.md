...
- **If the lookup returns empty** (keyring locked or entry missing), ask me to store it with
  `! secret-tool store --label='Figma Claude token' service figma account claude`, which reads the
...
- The token is **read-only** (scopes: `file_content`, `file_comments`, `file_versions`,
  `library_content`, `file_dev_resources`, `projects`), so don't attempt write operations with it.
...
| Endpoint | Purpose |
|---|---|
| `/v1/files/:fileKey?depth=1` | File overview (name, pages). Raise `depth` for more; full trees can be huge, so prefer `depth`/`ids` filters |
...
- **Known limitation:** the API returns `404 Not found` for files not explicitly shared with the
  token's account, and link-only view access is not enough. The file owner must invite my account
