...
- **Prefer `errgroup` where it fits, but it isn't the only allowed form of concurrency.** For a
...
- **With partial dependencies, parallelize the chains; don't serialize the whole function.**
  ...
  then a group for `B` and `C`, because that makes `C` wait on `A` for nothing. Group calls into
  dependency chains, keep the order only *within* each chain, run every chain concurrently.

- **Worth knowing (not a checklist):**

  - `errgroup.WithContext(ctx)` is what cancels the siblings on the first failure, so pass its
  ...
  - **You can collect every error, not just the first.** `g.Wait()` returns the first one, but
    what each goroutine records on the side is up to you: a channel, a mutex-guarded slice, a
    pre-sized `[]error` by index, whatever reads best, then `errors.Join(...)`. Skip `WithContext`
    there, or the first failure cancels the rest and you collect `context.Canceled` instead of
    real errors.
