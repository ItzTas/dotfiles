...
- `gitleaks` — best for secrets, and the only one that scans full git **history** well
  (`gitleaks detect` for history, `gitleaks dir` / `--no-git` for the working tree).
- `trufflehog` — good for **verified** secrets (`trufflehog filesystem <path>` or
  `trufflehog git file://.`).
...
- **Treat it as compromised — recommend rotating/revoking it immediately**, regardless of removal.
- Remove it from tracking and history: `git rm --cached`, add the pattern to `.gitignore`, and for
  ...
  only do it after my explicit confirmation.
- Prefer moving the value to **runtime retrieval** (fetch from `gh`/keyring/secret manager at
  runtime) instead of any plaintext config — matches how I like to handle secrets.
...
## 7. Summary
- End with a one-line verdict: **clean**, or **N secret(s) + M vulnerability finding(s)** — and the
