...
Optional argument (`$ARGUMENTS`): the bump — `major`, `minor`, `patch`, or an explicit version like
`1.4.0`. If omitted, infer it from the Conventional Commits since the last tag.
...
meant to be triggered (e.g. "this repo uses release-please — releases happen by merging its release
...
## 7. Report
- Summarize: previous → new version, how the bump was decided, the tag, and the release link(s).
