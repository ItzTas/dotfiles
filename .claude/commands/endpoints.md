...
- `bruno` is a Bruno collection: a `bruno/` folder in the repository with the endpoints as **YAML**
  request files (Bruno's YAML format, not `.bru`) + `bruno.json` + environment files.
- `insomnia` is an Insomnia export file (importable JSON/YAML).
- `postman` is a Postman Collection v2.1 JSON (+ a Postman environment file).
...
**Flags**, where single-dash and single-letter forms are equivalent: `--filter` = `-filter` = `-f`,
`--dry-run` = `-dry-run` = `-d`, `--out` = `-out` = `-o`, `--check` = `-check` = `-c`.

- `--filter <prefix|tag>` only includes the endpoints matching that path prefix or tag
  (e.g. `--filter /users`); useful in large APIs.
- `--dry-run` only discovers and **lists** the endpoints (method + path + params + auth) so I can
  review them; **write nothing**.
- `--out <dir>` writes the output under `<dir>` instead of the repo-root defaults.
- `--check` is CI/drift mode: **write nothing**; compare the existing collection(s) against the
  code and report the drift, meaning endpoints in the code that are missing from the collection,
  orphan requests that no longer exist in the code, and outdated bodies/params/auth. Report clean vs
...
- `--dry-run` and `--check` change the flow: do step 2 (discovery) normally, then follow the
  flag's behavior instead of steps 3 and 4. **No files are written** in either mode.
...
- **Ground everything in the code**, and don't invent endpoints, params, or fields. If you infer a
  body shape, say so.
...
- Use an environment variable for the base URL (`{{base_url}}` or the target's equivalent), and never
...
- Put **auth** (bearer token, API key, etc.) in collection-level auth / environment variables, not
  repeated per request. Use placeholder values, and **never copy real secrets** into the collection.
...
- **Check for an existing collection first** (a `bruno.json`/`.bru` folder, an Insomnia export, a
  Postman collection already in the repo) and **update it in place** rather than creating a
