...
- **Protected branches** (per my `CLAUDE.md`): `main`, `master`, `dev`, `develop`, `pre-homolog`,
...
- **Merged** — branches fully merged into the base: `git branch --merged <base>`. These are safe to
...
- Merged branches: `git branch -d <branch>` (aborts if not truly merged — good).
